...
@export_range(0.1, 1.0, 0.1, "or_greater", "suffix:px") var point_radius: float = 4.0
@export var enabled_point_color: Color = Color('00ff00')
@export var disabled_point_color = Color('ff0000')
@export var edge_color = Color('0000ff')
@export_range(0.1, 1.0, 0.1, "or_greater", "suffix:px") var edge_width: float = 1.0
...
func _draw():
    ...
    for point_id in _map.get_point_ids():
        ...
        for neighbor_point_id in _map.get_point_connections(point_id):
            ...
            if point_position.distance_squared_to(neighbor_position) == 1:
                draw_line(
                        point_position,
                        neighbor_position,
                        edge_color,
